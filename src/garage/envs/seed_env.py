...
class SeedEnv(Environment):
    ...
    def reset(self):
        ...
        if self._total_step_cnt%100 ==0: 
            ...
            self.event = filter_start_first_layer(self.event)
        ...
        self.state = [start_hit.z, start_hit.r]
        self.module_id = start_hit.discrete_module_id
    ...
    def step(self, action):
        ...
        #predicted_point_r = np.clip(a_clipped[1] + naive_estimate[1], 0, 100)

        predicted_point_z = a_clipped[0]
        predicted_point_r = a_clipped[1]
...
def filter_start_first_layer(hits): 
    pids_to_keep =  [] 
    for pid in np.unique(hits.particle_id): 
        p = hits[hits['particle_id']==pid]
        first = p.iloc[0, ]
        if (first['volume_id'] == 8) & (first['layer_id']==2): 
            pids_to_keep.append(first['particle_id'])
    
    return hits[hits['particle_id'].isin(pids_to_keep)]