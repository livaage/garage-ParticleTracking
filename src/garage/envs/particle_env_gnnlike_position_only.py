...
class ParticleEnvGnnLikePositionOnly(Environment):
    ...
    def __init__(self,
       ...
       # assert ((goal >= -arena_size) & (goal <= arena_size)).all()
        ...
        self._observation_space = akro.Box(low=np.array([0, 0]), high=np.array([266, 26]), dtype=np.float64)
        self._action_space = akro.Box(low=np.array([0, 0]),
                                      high=np.array([50,5]),
                                      shape=(2, ),
                                      dtype=np.float32)
    ...
    def reset(self):
        ...
        #self.event = self.event.sort_values(['r'],ascending=True).groupby('particle_id').head()
        self.event = self.event.sort_values('r')
        ...
        #observation = np.concatenate((self._point))
        #print(observation)
    ...
    def step(self, action):
        ...
        #                      266)
        a_clipped = [np.clip(a[0], self.action_space.low[0], self.action_space.high[0]),
        ...
        #predicted_point_z = np.clip(self._point[0] + a_clipped[0], 0, 266)
        #predicted_point_r = np.clip(self._point[1] + a_clipped[1], 0, 26) 
