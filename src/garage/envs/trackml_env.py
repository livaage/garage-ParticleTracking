...
with open("/home/lhv14/data.json", 'r') as j:
     allowed_modules_dr_dz = json.loads(j.read())
...
class TrackMLEnv(Environment):
    ...
    def reset(self):
        ...
        self.state = next_hit.squeeze(axis=0) 
        ...
        #self.allowed_next_modules = detector[detector['discrete_module_id'].isin(module_map[str(int(self.module_id))])]
        #self.allowed_next_modules = module_map[str(int(self.module_id))]        
    ...
    def step(self, action):
        ...
        allowed_modules = allowed_modules_dr_dz[str(int(self.module_id))]
        #v = np.array(list(d['12275'].values()))[:, 0:2]
        drdz_allowed = np.array(list(allowed_modules.values()))[:, 0:2]
        d = drdz_allowed - [self.dz_buffer[-1], self.dr_buffer[-1]]
        index_best_module = np.argmin([np.sqrt(val[0]**2 + val[1]**2) for val in d])
        best_mod = list(allowed_modules.keys())[index_best_module]

        pos_best_mod = detector[detector['discrete_module_id'] ==int(float(best_mod))][['cz', 'cr']].values[0]
        #nearest_module, module_id = find_nearest_module(naive_estimate[0], naive_estimate[1], self.module_id)
        #print(pos_best_mod)
        predicted_point_z = np.clip(a_clipped[0] + pos_best_mod[0], -300, 300)
        predicted_point_r = np.clip(a_clipped[1] + pos_best_mod[1], 0, 100)

        #nearest_module, module_id = find_nearest_module(predicted_point_z, predicted_point_r, self.module_id)


        #naive_estimate = self._point + np.array([max(0, self.dz_buffer[-1]),  max(0, self.dr_buffer[-1])])
        #nearest_module, module_id = find_nearest_module(naive_estimate[0], naive_estimate[1], self.module_id)
        #nearest_module_point = detector[detector['new_module_id'] ==  nearest_module][['cz', 'cr']].values.flatten()
        self.module_id = int(float(best_mod)) 
        ...
        #predicted_point = [nearest_module[0], nearest_module[1]]
        ...
        #print(self.allowed_next_modules.shape)
...
def find_nearest_module(pred_z, pred_r, prev_module):
    ...
    #prev_module_pos = 
    compatible_modules = detector[detector['discrete_module_id'].isin(module_map[str(int(prev_module))])]
    #print(prev_module, compatible_modules)
    
    # only radially outwards
    #print(compatible_modules)
    compatible_modules = compatible_modules[(np.abs(compatible_modules['cz']) > np.abs((pred_z))) & (compatible_modules['cr'] > (pred_r))]
    if len(compatible_modules) == 0: 
            compatible_modules = detector[detector['discrete_module_id'].isin(module_map[str(int(prev_module))])]
    ...
    module = compatible_modules.iloc[index, ]
    module_position = module[['cz', 'cr']].values
    module_id = module.discrete_module_id

    #return compatible_modules['new_module_id'].values[index]
    
    #print(prev_module, " connected to ", module_id, "at pred ", pred_z, pred_r, " with module at ", module_position)
