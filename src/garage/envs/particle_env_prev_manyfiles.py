...
class ParticleEnvPrevManyFiles(Environment):
    ...
    def __init__(self,
        ...
        self._observation_space = akro.Box(low=np.array([-266, 0, -100, -10]), high=np.array([266, 26, 20,10]), dtype=np.float64)
    ...
    def reset(self):
        ...
        observation = np.concatenate((self._point, dist))
    ...
    def step(self, action):
        ...
        if (self._total_step_cnt ==10001) & (self.done_visual == False) : 
        ...
        observation = np.concatenate((self._point, distance_prev_hit))
