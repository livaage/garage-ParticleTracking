...
class ParticleEnvSimple(Environment):
    ...
    def reset(self):
        ...
        if (self._total_step_cnt%10 ==0) & (self._total_step_cnt!=0): 
            self.file_counter += 1 
    ...
    def step(self, action):
        ...
        #self.file_counter.append(self.file_counter)

        self._step_cnt += 1
        self._total_step_cnt += 1
        #print(self._step_cnt)

        if (self._total_step_cnt > 15000) & (self._total_step_cnt < 15002): 
        ...
        observation = np.concatenate((self._point, [np.mean(self.dz_buffer)], [np.mean(self.dr_buffer)]))
