...
class ParticleEnvGnnLike(Environment):
    ...
    def __init__(self,
       ...
       # assert ((goal >= -arena_size) & (goal <= arena_size)).all()
        ...
        self._action_space = akro.Box(low=np.array([-150, 0]),
    ...
    def step(self, action):
        ...
        predicted_point_z = np.clip(self._point[0] + a_clipped[0], -266, 266)
        predicted_point_r = np.clip(self._point[1] + a_clipped[1], 0, 27)
