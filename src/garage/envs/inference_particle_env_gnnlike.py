...
class InferenceParticleEnvGnnLike(Environment):
    ...
    def __init__(self,
        ...
        self.file_counter = 0 
        self.event = pd.read_hdf('~/gnnfiles/data/ntuple_PU200_numEvent1000/ntuple_PU200_event100.h5')
    ...
    def reset(self):
        ...
        if self._total_step_cnt%1000 ==0: 
            self.file_counter += 1 
            self.event = pd.read_hdf('~/gnnfiles/data/ntuple_PU200_numEvent1000/ntuple_PU200_event'+str(self.file_counter+100)+'.h5')
            print("jumping file")
        ...
        self.event = self.event[self.event['particle_id'].isin(np.array(nh[nh > 1].index))]

        random_particle_id = random.choice(self.event.particle_id.values)
        print("The file is,", str(self.file_counter+100) )
        print("The particle is ",random_particle_id)
    ...
    def step(self, action):
        ...
        print("the actions is actually", a)
        #print(a)
        ...
        #if (self._total_step_cnt > 200000) & (self._total_step_cnt < 200002): 
        ...
        if (self.num_track_hits > 15) or (stopping <0.1):
            bonus_reward = np.abs(self.num_track_hits - len(self.original_particle))
            #reward = reward - bonus_reward *100
            done = True 
        else: 
            done = False 
        ...
        observation = np.concatenate((self._point, [np.mean(self.dz_buffer)], [np.mean(self.dr_buffer)], [dphi], [deta]))
