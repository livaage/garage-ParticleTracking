...
def add_point(z, r, dt): 
    
    circles.append(shapes.Circle(z, r, 15, color=(250,60,60), batch=batch))



def visualise_particle(particle_id): 
    ...
    e_particle = event[event['particle_id']==particle_id] 
    ...
    #circles = [] 
    for i in range(len(e_particle)): 
        hit = e_particle.iloc[i,:] 
        circles.append(shapes.Circle(hit.z, hit.r, 15, color=(60,60,60), batch=batch)) 
...
#       add_point()        
    ...
    pyglet.clock.schedule_interval(add_point, 1, 1, 3)
